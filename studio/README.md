...
用户核心诉求：**不是"基于原场景"，而是从"静态资产 + 动态资产 + 道路数据"合成符合现实的
场景/事故**。下面四项都是朝这个思路做的：

### 1. 时间轴延伸沿车道走（不再是直线）

`track_manager._extrapolate_poses_smart`：有高精地图时，先 `road_rules.RoadModel.route_forward`
沿**车道中心线**外推（遇分叉选"最接近当前航向"的出口继续走），位置取车道、航向取车道方向；
没有地图/不在车道上才退回直线外推。实测 scene 016 环岛：自车延伸段航向从 -11° 平滑转到 +68°，
沿环岛弧度拐，不再直冲出去。`route_forward` 与 `plan_along_lane` 的区别就是**遇分叉不停**。

### 2. 转向/掉头严格受交通标线规则约束

* `turn` 操作只走车道图：`plan_turn` 用"整条候选路径的净航向变化"判定左右/直行/掉头，
  找不到匹配支路就**拒绝执行**（`ok:false` + 原因），**不再有"原地把朝向转过去"的兜底**
  —— 那等于"随便哪都能拐"。只有"没有高精地图"时才退回原地改朝向（无从谈规则）。
* 新增 **掉头**：`direction:"uturn"`，目标 ±180°，用角度的绝对大小匹配。
* 变道终点早已吸附到"地图里真实相邻车道"的中心线（`neighbor_lane`，含几何兜底）。

### 3. 动态物体资产化：漏检帧不消失

单目检测偶发漏掉某一帧 → 之前那帧的逐帧元数据里没有该物体，整帧消失。现在：

* `get_track_pose` 在**首末检测之间**做相邻帧位姿插值（末次检测之后不夹取，物体可能真走了）；
* `get_frame_objects` 把"检测区间覆盖这一帧、但这一帧没识别到"的物体补回来（`persisted:true`）；
* `build_extra_objects` 为这些物体生成"复用最后一次外观 + 插值位姿"的渲染项，donor 用
  "这一帧之前最近一次检测"的 raw id（渲染是顺序的，它的外观一定已在缓存里）；
* 渲染器 donor 克隆在当帧 PLY 里找不到该 id 时，回退到 `_obj_appearance` 外观缓存。

实测 scene 016 的 T5（检测帧 6..13、20..24，中间 14..19 漏检）：frame 14 现在照常列出、
照常渲染（donor=帧13 的 raw id）。

### 4. 碰撞不乱飞 + SAM3D 朝向修正

* 碰撞冲量**大幅限幅**（restitution 0.15→0.05，肇事车速度增量 ≤8→2.5 m/s、受害车 ≤12→4 m/s），
  双方碰后快速摩擦减速停下；**车头始终朝运动方向**（`_pose_facing`），不再横着滑/飞。
* SAM3D 导入朝向不再假设固定 canonical：`_ply_bbox_and_orientation` 用 **PCA** 找模型自身的
  "最短轴=高、最长水平轴=长"，把真实长轴对齐到 DGGT 前进方向（det=+1 正交旋转），
  模型不再斜着/横着摆。
...
## 批量生成质量攻坚：从"整批报废"到 64/64 全有效

用户反馈："生成的事故轨迹乱七八糟、事故参与者消失，完全不能用来生产 corner case"。
本轮按"端到端复现 → 定位根因 → 修根因"推进，**没有靠调质量门的阈值来粉饰**。
修复前 11 种事故类型在 scene 018 上几乎全废；修复后 **3 个场景 × 11 类型 × 2 例 = 64/64 全部有效**。

### 根因 1：逐帧"读-改-写" + 自动朝向 → 车头 180° 翻转（"轨迹乱七八糟"的主因）

`prim_regularize` / `prim_lateral_sweep` / `prim_hold_still` / `_apply_initial_pair_layout` /
`separate_pair` / `_apply_brake_decel` 原先都是**逐帧 `get_track_pose` → 改位置 → 写回**。
问题在于 `get_track_pose` 会应用 `_apply_auto_heading`（按轨迹运动方向纠正车头），于是写到第 0 帧后：
f0 的位置已经变了、f1..fN 还是旧值，重新估算运动方向会得到**恰好反向**的结果 →
车头 180° 翻转、`yaw_rate` 直接 31 rad/s。

**修法**：全部改成"先把目标位姿算完、再一次性写回"。批量写回后路径形状自洽，自动朝向不再误判。
另外静止物体的朝向锁定在首帧（`prim_regularize`/`prim_hold_still`/`_apply_initial_pair_layout`），
避免原始检测噪声让"停着的车"车头乱转。

### 根因 2：自然冲突沿带噪折线重采样 → 一顿一顿（accel 22 / jerk 62）

`natural_conflict.extract_path` 的 `frame_arc` 原先是"每个帧点投影到折线后的**最近顶点弧长**"，
这是量化台阶：相邻帧可能落在同一顶点、再整段跳一次。`retime_track` 按它重采样 →
速度剖面一顿一顿，`max_accel` 22 m/s²、`max_jerk` 62 m/s³。

**修法**：① 对逐帧中心点做 `[1,2,1]/4` 的轻度平滑（端点固定）去掉检测噪声；
② `frame_arc` 改成**平滑后的逐帧累积弧长**（连续、单调），不再用量化台阶。

### 根因 3：合成参与者朝向 = 锚点朝向 → 对向车"猛掉头"

`_synth_initial_poses` 给合成参与者的旋转直接沿用锚点位姿，而它的运动方向可能是**反向**的
（对向车 `heading = -anchor_head`）。`_apply_auto_heading` 随后在头几帧把车头硬转 180°，
`yaw_rate` 达到 14 rad/s。

**修法**：合成位姿的朝向直接对齐运动方向（`_pose_facing(base, c, heading*speed)`）；静止时锁定锚点朝向。

### 根因 4：仿真朝向 ≠ 写入位置 → 单帧猛转 37°/80°

`_simulate_pursuit_collision` 里肇事车的朝向按**仿真速度方向**算（还会朝拦截点转向），
但写入位置额外叠加了横向偏移和接触约束，两者不一致时某一帧朝向可跳 37°~80°
（`yaw_rate` 6~14 rad/s）。真实的车不可能单帧转 80°。

**修法**：新增 `_slew_pose_yaw`，写入前把逐帧偏航角限制为**最多转 5°**
（10fps → 0.87 rad/s，安全落在 1.2 rad/s 阈值内），位置完全不变。

### 根因 5：参与者"整段消失" —— donor 外观缓存没预热

渲染器按帧顺序加载 PLY 并顺手缓存每个物体的外观（`_obj_appearance`）。合成参与者靠
**克隆 donor 高斯**渲染；若该帧的 PLY 里没有 donor、且缓存里也还没有 → 渲染器直接 `continue`，
**这个参与者在画面里整段不见**。合成参与者常出现在窗口最开头，而 donor 可能到窗口中部才被检测到。

实测复现（016，donor 首帧=24）：合成参与者在 **f0–f11 整整 12 帧完全不渲染**。
**修法**：新增 `TrackManager.prewarm_render_appearance()`，渲染前按"每个 donor 首次出现的帧"
预热缓存；`render_sequence_video` 出片前自动调用。修复后缺帧 = 0。

另外 `build_extra_objects` 的补帧范围原先只认 `fmin..fmax`，编辑帧落在检测跨度之外的参与者
会在窗口两端消失；现在**编辑帧一律补**。

### 根因 6：候选"窗口不匹配" + 首帧无检测 → 整类 0 条

* `natural-conflict` 的候选按"两条轨迹真的会交汇"筛（`_natural_conflict_candidates` 穷举真实车辆
  两两配对，最近距离 ≤ 4.5m 且最大间距 ≥ 6m —— 后者排除"同一物体被跟踪成两个 id"）。但批量里
  场景图按 `frame_window=10` 建、生成窗口却是 25 帧，窄窗口里还没交汇 → 永远 0 候选。
  **修法**：场景图按 `max(frame_window, num_frames)` 建。
* 场景 016 的逻辑首帧**没有任何检测**（车辆要到第 7 帧才出现），于是所有物体 `usable=False`、
  全类型 0 候选、批量静默产出 0 条。**修法**：批量前用 `_first_usable_anchor_frame` 在窗口开头
  附近找"有 ≥2 辆车的最早帧"作为生成起点（日志 + 响应的 `start_frame` 都会说明自动前移）。

### 质量门不再"假装没有事件"

`quality_report` 对 `natural-conflict` 采用"两条**真实**轨迹交汇到 ≤4.5m"作为事件判据
（`SCENARIO_NEAR_MISS_DISTANCE`）：纯时序重定时受物理限幅约束，未必能把两车压到 2m 以内，
但最近接近帧 + TTC 仍然是有意义的 corner case。`cut-out-reveal`（露出障碍，本就不该撞）
与 `hard-brake`（单车风险）继续判为"无碰撞事件，不适用"。

### 验证方式（全部数值化，不靠看图）

| 检查 | 修复前 | 修复后 |
| --- | --- | --- |
| scene 018 全 11 类型 `valid` | 多数 false | **22/22** |
| scene 017 全 11 类型 `valid` | — | **22/22** |
| scene 016 全 11 类型 `valid` | 全类型 0 候选 | **20/20** |
| `max_step_distance`（追尾） | 5.45 m | 2.5 m |
| 合成参与者 yaw_rate | 31.4 rad/s | 0.0 rad/s |
| 自然冲突 max_accel / jerk | 22.3 / 62.5 | 6.7 / 13.5 |
| donor 首帧=24 时的渲染缺帧 | **f0–f11 共 12 帧** | 0 帧 |
| 仿真车 max_yaw_rate | 14.0 rad/s | 0.87 rad/s |
