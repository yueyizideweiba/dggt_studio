...
def save_ply(points, scales, rotations, opacities, colors, path):
    points = _to_numpy(points)
    scales = _to_numpy(scales)
    rotations = _to_numpy(rotations)
    ...
    if scales.shape[1] < 3:
        scales = np.pad(scales, ((0, 0), (0, 3 - scales.shape[1])), mode='constant')
    rotations = rotations.reshape(n, -1)
    if rotations.shape[1] < 4:
        rotations = np.pad(rotations, ((0, 0), (0, 4 - rotations.shape[1])), mode='constant')
    ...
    with open(path, 'w') as f:
        ...
        for prop in ['x', 'y', 'z', 'nx', 'ny', 'nz', 'f_dc_0', 'f_dc_1', 'f_dc_2', 'opacity', 'scale_0', 'scale_1', 'scale_2', 'rot_0', 'rot_1', 'rot_2', 'rot_3']:
...
def main():
    ...
    parser.add_argument('-difix', action='store_true', help='Whether to process images with Difix refinement')
        ...
        for batch in dataloader:
            ...
            with torch.cuda.amp.autocast(dtype=dtype):
                ...
                if args.mode == 3:
                    ...
                    static_rgbs, static_opacity, static_scales, static_rotations = get_split_gs(bg_gs_map, static_mask)
                    frame_idx = torch.nonzero(static_mask, as_tuple=False)[:,1]
                    gs_timestamps = timestamps[frame_idx]
                    static_opacity = static_opacity * (1 - gs_dynamic_list)
            ...
            export_dir = os.path.join(scene_out_dir, "exports")
            dynamic_dir = os.path.join(export_dir, "dynamic")
            static_dir = os.path.join(export_dir, "static")
            sky_dir = os.path.join(export_dir, "sky")
            ego_dir = os.path.join(export_dir, "ego_pose")
            os.makedirs(dynamic_dir, exist_ok=True)
            os.makedirs(static_dir, exist_ok=True)
            os.makedirs(sky_dir, exist_ok=True)
            os.makedirs(ego_dir, exist_ok=True)

            # Export per-frame geometry and pose products for downstream rendering/editing.
            def _export_frame_geometry(frame_idx, frame_tag):
                frame_dir = os.path.join(dynamic_dir, frame_tag)
                os.makedirs(frame_dir, exist_ok=True)

                ext_t = extrinsic[frame_idx]
                c2w_t = torch.inverse(ext_t)
                ego_payload = {
                    "frame_id": int(frame_idx),
                    "timestamp": float(timestamps[frame_idx].detach().cpu().item()) if torch.is_tensor(timestamps[frame_idx]) else float(timestamps[frame_idx]),
                    "camera": {"width": int(W), "height": int(H)},
                    "intrinsics": intrinsic[frame_idx].detach().cpu().tolist(),
                    "extrinsics_w2c": ext_t.detach().cpu().tolist(),
                    "extrinsics_c2w": c2w_t.detach().cpu().tolist(),
                }
                save_json(ego_payload, os.path.join(ego_dir, f"{frame_tag}.json"))

                if frame_idx < len(dynamic_points) and dynamic_points[frame_idx].numel() > 0:
                    save_ply(
                        ...
                        os.path.join(frame_dir, "dynamic.ply"),
                    )

            for frame_idx in range(len(dynamic_points)):
                _export_frame_geometry(frame_idx, f"frame_{frame_idx:04d}")
            ...
            if static_points.numel() > 0:
                save_ply(
                    ...
                    static_opacity,
                    static_rgbs,
                    os.path.join(static_dir, "background_static.ply"),
                )

            if args.mode == 2:
                sky_points = model.sky_model.bg_pcd.detach().cpu()
                sky_scales = torch.exp(model.sky_model.bg_scales).detach().cpu()
                sky_rotations = model.sky_model.bg_quat.detach().cpu()
                sky_opacities = model.sky_model.bg_opacity.squeeze(-1).detach().cpu()
                sky_colors = model.sky_model.bg_color.detach().cpu() if hasattr(model.sky_model, "bg_color") else torch.ones_like(sky_points)
                save_ply(
                    sky_points,
                    sky_scales,
                    sky_rotations,
